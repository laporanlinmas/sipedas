...
function CameraCell({
  ...
  const getNextUrl = () => {
    ...
    return `${ch.url}${sep}_t=${Date.now()}`;
  };

  const scheduleNext = () => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    // Faster refresh for "lancar" feel, but adaptive
    timeoutRef.current = setTimeout(() => {
      const next = getNextUrl();
      if (activeBuffer === 0) setSrc1(next);
      else setSrc0(next);
    }, 150); 
  };
  ...
  const handleLoad = (bufferIdx: number) => {
    if (loadingRef.current) {
      loadingRef.current = false;
      setLoading(false);
    }
    
    // Swap buffer ONLY if the loaded one is the "next" one
    if (bufferIdx !== activeBuffer) {
  ...
  const handleError = () => {
    if (loadingRef.current) {
      ...
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
      timeoutRef.current = setTimeout(scheduleNext, 2000);
    }
  };
  ...
  const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (touches && touches.length === 2 && lastDist > 0) {
      const d = Math.hypot(touches[0].clientX - touches[1].clientX, touches[0].clientY - touches[1].clientY);
      const factor = d / lastDist;
      setZoom(prev => Math.max(1, Math.min(5, prev * factor)));
      setLastDist(d);
      return;
    }
    ...
    if (err) {
      return (
        <div className="cctv-error">
          ...
          <button onClick={(e) => { e.stopPropagation(); setErr(false); setLoading(true); scheduleNext(); }}>
    ...
    return (
      ...
      >
        <img
          src={src0}
          alt=""
          style={{ ...imgStyle, opacity: activeBuffer === 0 ? 1 : 0, zIndex: activeBuffer === 0 ? 2 : 1 }}
          onLoad={() => handleLoad(0)}
          onError={handleError}
        />
        <img
          src={src1}
          alt=""
          style={{ ...imgStyle, opacity: activeBuffer === 1 ? 1 : 0, zIndex: activeBuffer === 1 ? 2 : 1 }}
          onLoad={() => handleLoad(1)}
          onError={handleError}
        />
      </div>
  ...
  return (
    ...
    >
      ...
      {/* Zoom Controls */}
      {isFocused && ch.active && ch.url && !err && (
        <div className="cctv-zoom-ctrl">
          <button onClick={(e) => { e.stopPropagation(); handleZoom(0.5); }} title="Zoom In">
            <i className="fas fa-plus" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); handleZoom(-0.5); }} title="Zoom Out">
            <i className="fas fa-minus" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); setZoom(1); setOffset({x:0,y:0}); }} title="Reset Zoom">
      ...
      {showOverlay && (
        <div className="cctv-cell-overlay">
          <div className="cctv-cell-badge">
            ...
            <span>CH {ch.id} {zoom > 1 && `· ${zoom.toFixed(1)}x`}</span>
          </div>
